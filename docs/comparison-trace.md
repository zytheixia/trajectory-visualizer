...
## 为什么需要对齐线 (ComparisonAnchor)

多轨迹对比不能简单按数组 index 或绝对时间戳机械对齐。真正的对比价值在于通过外部对比算法，将多条 trace 在逻辑上平行的节点在视觉纵轴上拉平：
...
`ComparisonAnchor` 就是独立于各轨迹数据之外的**跨轨道对齐参考线**。它指定了哪些节点应该在视觉纵向上垂直拉平，从而凸显不同 Run 之间的耗时差异与顺序重排。
